...
class NcafeWriteAtt :
    ...
    def __init__(self) :
        ...
        self.driver =  webdriver.Chrome(chrome_options=option, executable_path=r"D:/atom_py/section3/webdriver/Chrome/chromedriver")
        self.driver.implicitly_wait(5)

    def writeAttendCheck(self) :
        self.driver.get('https://nid.naver.com/nidlogin.login')

        pyperclip.copy('dreaming91')   #아이디입력
        self.driver.find_element_by_name('id').click()
        ActionChains(self.driver).key_down(Keys.CONTROL).send_keys('v').key_up(Keys.CONTROL).perform()


        pyperclip.copy('no2510')   #비밀번호 입력
        ...
        # self.driver.find_element_by_xpath('//*[@id="cmtinput"]').send_keys('반갑습니다~ ~~~~')
        # self.driver.find_element_by_xpath('//*[@id="btn-submit-attendance"]').click()
        soup= BeautifulSoup(self.driver.page_source,'html.patser')
        #선택자 추출
...
def printMemberList(self,list):
    f= opne("D:/atom_py/section3/webdriver/chrome/memberList.txt",'wt')
    for i in list:
